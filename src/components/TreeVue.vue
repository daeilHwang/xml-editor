                ...
                async getChildren(id) {
                    ...
                    if (!id) {
                      for(let key in _helper.getXmlData()){
                          if(key !== '?xml')
                                transactions.push({ id: key, label: '<' + key + '>', icon: 'fa-arrow-alt-circle-left', state: 'collapsed' });
                      }
                    } else {
                          ...
                          for(let key in instance){
                            if(Array.isArray(instance[key])){
                              ...
                              if(key.indexOf('#text') != -1){
                                ...
                                transactions.push({ id: id + '_' + key, label: _helper.makeLabel(key, instance[key]), icon: 'fa-folder', state: 'collapsed' });
                            }
                          }
                          break;
                        default:
